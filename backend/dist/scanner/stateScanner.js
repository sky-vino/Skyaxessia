...
exports.runStateScanning = runStateScanning;
...
async function screenshotBase64(page, beforeSnapshot) {
    try {
        if (beforeSnapshot)
...
// ── Hover state ───────────────────────────────────────────────────────────────
async function testHoverState(page, url, beforeSnapshot) {
    const issues = [];
    try {
        const hoverTargets = await page.evaluate(() => Array.from(document.querySelectorAll("a[href],button,nav a,[role='button']"))
            .slice(0, 15)
        ...
        for (const sel of hoverTargets) {
            try {
                ...
                await (0, navigation_1.waitForStability)(page, 300);
                // Run axe on the hovered state
                const axeIssues = await (0, axeScan_1.runAxe)(page, url, "hover", "hover");
        ...
        const tooltipIssues = await page.evaluate(() => {
            const out = [];
            document.querySelectorAll("[title],[data-tooltip],[aria-describedby]").forEach((el) => {
        ...
        logger_1.logger.debug("Hover state test failed:", err);
    }
...
// ── Focus state ───────────────────────────────────────────────────────────────
async function testFocusState(page, url, beforeSnapshot) {
    const issues = [];
    try {
        const focusTargets = await page.evaluate(() => Array.from(document.querySelectorAll("a[href],button,input,select,textarea,[tabindex='0']"))
            .slice(0, 10)
            .map((el) => el.id ? `${el.tagName.toLowerCase()}#${el.id}` : el.tagName.toLowerCase()));
        if (focusTargets.length > 0) {
            try {
                await page.focus(focusTargets[0]);
            }
            catch { }
            await (0, navigation_1.waitForStability)(page, 400);
            const focusAxe = await (0, axeScan_1.runAxe)(page, url, "focus", "focus");
            issues.push(...focusAxe.map(i => ({ ...i, state: "focus", phase: "focus" })));
        }
    }
    catch (err) {
        logger_1.logger.debug("Focus state test failed:", err);
    }
    const a11yTree = await accessibilityTree(page);
    const screenshot = await screenshotBase64(page, beforeSnapshot);
...
async function testErrorState(page, url, beforeSnapshot) {
    const issues = [];
    try {
        const forms = await page.evaluate(() => Array.from(document.querySelectorAll("form")).map((f) => f.id ? `form#${f.id}` : "form").slice(0, 3));
        for (const formSel of forms) {
            try {
                ...
                await (0, navigation_1.waitForStability)(page, 800);
                ...
                const errorIssues = await page.evaluate(() => {
                    ...
                    document.querySelectorAll(".error,.error-message,.field-error,[role='alert']").forEach((el) => {
                ...
                const axeErrors = await (0, axeScan_1.runAxe)(page, url, "error", "error");
        ...
        logger_1.logger.debug("Error state test failed:", err);
    }
    const a11yTree = await accessibilityTree(page);
    const screenshot = await screenshotBase64(page, beforeSnapshot);
...
async function testExpandedState(page, url, beforeSnapshot) {
    const issues = [];
    try {
        ...
        for (const exp of expandables) {
            try {
                ...
                const expandedIssues = await page.evaluate((type) => {
                    const out = [];
                    // Check opened menus
                    ...
                    document.querySelectorAll("[role='dialog'][aria-modal='true']").forEach((modal) => {
                        ...
                        if (st.display !== "none") {
                            const firstFocusable = modal.querySelector("button,a,[tabindex='0'],input");
                ...
                const axeExpanded = await (0, axeScan_1.runAxe)(page, url, "expanded", "expanded");
        ...
        logger_1.logger.debug("Expanded state test failed:", err);
    }
    const a11yTree = await accessibilityTree(page);
    const screenshot = await screenshotBase64(page, beforeSnapshot);
...
async function testTabPanels(page, url, beforeSnapshot) {
    const results = [];
    try {
        ...
        for (let i = 1; i < tabCount; i++) {
            ...
            try {
                ...
                const selector = await tab.evaluate((el, idx) => {
                    if (el.id)
                        return `[role='tab']#${CSS.escape(el.id)}`;
                ...
                await (0, navigation_1.waitForStability)(page, 800);
                const panelVisible = await page.evaluate((idx) => {
                ...
                const axeIssues = await (0, axeScan_1.runAxe)(page, url, `tab ${i + 1}`, `tab-${i + 1}`);
...
async function testCustomState(page, url, stateConfig, beforeSnapshot) {
    const issues = [];
    try {
        ...
        await (0, navigation_1.waitForStability)(page, stateConfig.waitMs || 600);
        const axeIssues = await (0, axeScan_1.runAxe)(page, url, stateConfig.name, stateConfig.name);
        issues.push(...axeIssues.map(i => ({ ...i, state: stateConfig.name, phase: stateConfig.name })));
    }
    catch (err) {
        logger_1.logger.debug(`Custom state "${stateConfig.name}" test failed:`, err);
    }
    const a11yTree = await accessibilityTree(page);
    const screenshot = await screenshotBase64(page, beforeSnapshot);
...
// ── Main export ───────────────────────────────────────────────────────────────
async function runStateScanning(page, url, extraStates = [], depthMode = "standard", beforeSnapshot) {
    const results = [];
...
async function testDiscoveredButtonStates(page, url, beforeSnapshot) {
    const results = [];
    const targets = await page.evaluate(() => Array.from(document.querySelectorAll("button,a[href],[role='button'],[role='link']"))
        .filter((el) => {
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        try {
            ...
            await (0, navigation_1.waitForStability)(page, 250);
            const axeIssues = await (0, axeScan_1.runAxe)(page, url, stateName, stateName);

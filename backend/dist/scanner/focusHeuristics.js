...
exports.runFocusHeuristics = runFocusHeuristics;
...
async function runFocusHeuristics(page, url, state, phase) {
    const issues = [];
    // ── 1. Focus not visible ─────────────────────────────────────────────────
    const focusInvisible = await safeEval(page, () => {
        const out = [];
        const getP = (el) => {
            const path = [];
            let d = 0, c = el;
            while (c && c.nodeType === 1) {
                if (c.id) {
                    path.unshift(`${c.tagName.toLowerCase()}#${c.id}`);
                    break;
                }
        ...
        document.querySelectorAll("a[href],button,input,select,textarea,[tabindex]")
            ...
            if (preSt.display === "none")
                return;
            if (preSt.visibility === "hidden")
                return;
            if (parseFloat(preSt.opacity || "1") === 0)
                return;
            ...
            if (!hasVisibleOutline && !hasVisibleShadow)
                out.push(getP(el));
        });
    ...
    // ── 2. Focus obscured ────────────────────────────────────────────────────
    const focusObscured = await safeEval(page, () => {
        const out = [];
        const getP = (el) => {
            const path = [];
            let d = 0, c = el;
    ...
    const dialogIssues = await safeEval(page, () => {
        const noFocusable = [];
    ...
    try {
        ...
        logger_1.logger.debug("Escape key dialog test could not run:", err);
    }
    return issues;
}

...
exports.runColorChecks = runColorChecks;
...
async function runColorChecks(page, url, state, phase) {
    const issues = [];
    try {
        const contrastData = await page.evaluate(() => {
            ...
            const lum = (c) => {
                const s = (v) => { const n = v / 255; return n <= 0.03928 ? n / 12.92 : Math.pow((n + 0.055) / 1.055, 2.4); };
        ...
        if (contrastData.length > 0) {
            ...
            issues.push({
                ...
                state, phase,
                htmlSnippet: contrastData.slice(0, 5).map(d => `/* ${d.selector}: ${d.ratio}:1 (need ${d.isLarge ? 3 : 4.5}:1) fg=${d.fg} bg=${d.bg} */`).join("\n"),
            });
        }
        ...
        logger_1.logger.warn("Color contrast check failed:", err);
    }
    return issues;
}

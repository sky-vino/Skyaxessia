...
exports.runZoomChecks = runZoomChecks;
exports.runPointerChecks = runPointerChecks;
...
async function closeTransientUi(page) {
    ...
    await page.evaluate(() => {
        ...
        document.querySelectorAll("[role='menu'],[role='listbox'],[role='dialog'],[aria-modal='true'],.dropdown-menu,.menu,.popover,.tooltip,[data-popper-placement]").forEach((el) => {
...
async function restoreInjectedUi(page) {
    await page.evaluate(() => {
        document.getElementById("aft-text-spacing-test")?.remove();
        document.querySelectorAll("[data-aft-original-style]").forEach((el) => {
            const original = el.getAttribute("data-aft-original-style");
            if (original === "__AFT_NO_STYLE__")
        ...
        document.querySelectorAll("[data-aft-evidence-highlight]").forEach((el) => {
...
async function naturallyCloseTransientUi(page) {
    await page.keyboard.press("Escape").catch(() => undefined);
    await page.mouse.click(2, 2).catch(() => undefined);
    await page.waitForTimeout(150).catch(() => undefined);
}
async function captureUiSnapshot(page) {
    return page.evaluate(() => {
        const selectorFor = (el) => {
            ...
            const html = el;
            if (html.id)
                return `${html.tagName.toLowerCase()}#${CSS.escape(html.id)}`;
            const ariaLabel = html.getAttribute("aria-label");
            if (ariaLabel && ariaLabel.length < 80)
                return `${html.tagName.toLowerCase()}[aria-label="${CSS.escape(ariaLabel)}"]`;
            const controls = html.getAttribute("aria-controls");
            if (controls)
                return `${html.tagName.toLowerCase()}[aria-controls="${CSS.escape(controls)}"]`;
            ...
            if (parent) {
                ...
                if (siblings.length > 1)
                    selector += `:nth-of-type(${siblings.indexOf(html) + 1})`;
            }
            return selector;
        };
        const visible = (el) => {
        ...
        const visibleTransients = Array.from(document.querySelectorAll(transientSelector))
            .filter(visible).map(selectorFor).filter(Boolean);
...
async function screenshotEvidence(page, explanation) {
    try {
...
async function screenshotEvidenceForSelector(page, selector, explanation, resetUi = true) {
    if (resetUi)
...
async function runZoomChecks(page, url, state, phase, 
/**
...
zoomTargetPercent = 400) {
    ...
    if (!runFourHundredReflow) {
        logger_1.logger.info(`Zoom checks: 200% target selected for ${url} — skipping 320px reflow (WCAG 1.4.10 400% equivalent).`);
    }
    ...
    const fixedFonts = await page.evaluate(() => {
        const out = [];
        document.querySelectorAll("p,span,a,li,td,th,h1,h2,h3,h4,h5,h6,button,input,label").forEach((el) => {
            const fs = el.style?.fontSize || "";
            if (fs.endsWith("px") && parseFloat(fs) < 16)
                out.push(el.id ? `${el.tagName.toLowerCase()}#${CSS.escape(el.id)}` : el.tagName.toLowerCase());
        });
    ...
    try {
            ...
            const probe = await page.evaluate(() => {
                const selectorFor = (el) => {
                    ...
                    if (label && label.length < 60)
                        return `${el.tagName.toLowerCase()}[aria-label="${CSS.escape(label)}"]`;
                ...
                document.querySelectorAll("body *").forEach((el) => {
                    ...
                    if (!visible || style.position === "fixed" || style.position === "sticky")
                        return;
                    const hasText = (el.textContent || "").trim().length > 1;
                    const clipped = hasText && (el.scrollWidth > el.clientWidth + 2 || el.scrollHeight > el.clientHeight + 2 || style.textOverflow === "ellipsis" || Boolean(style.webkitLineClamp));
        ...
        if (intermediateFailures.length) {
            ...
            const evidence = await screenshotEvidenceForSelector(page, selector, `Issue-specific evidence captured at the ${first.zoomPercent}% equivalent viewport (${first.viewport.width}x${first.viewport.height}). The highlighted element is the first overflow or clipping offender.`, false);
            issues.push({
        ...
        const zoomData = await page.evaluate(() => {
            ...
            const selectorFor = (el) => {
                ...
                if (role)
                    part += `[role="${CSS.escape(role)}"]`;
                if (label && label.length < 60)
                    part += `[aria-label="${CSS.escape(label)}"]`;
                const parent = html.parentElement;
                if (!parent)
                    return part;
                const siblings = Array.from(parent.children).filter((child) => child.tagName === html.tagName);
                if (siblings.length > 1)
                    part += `:nth-of-type(${siblings.indexOf(html) + 1})`;
                return part;
            };
            const visible = (el) => {
                if (!el)
            ...
            const tableFailures = [];
            const focusFailures = [];
            const crowdedTargets = [];
            const missingFunctionalLabels = [];
            const scrollTraps = [];
            document.querySelectorAll("*").forEach((el) => {
                ...
                if (st.position !== "fixed" && st.position !== "sticky" && r.right > viewport.width + 2)
                    overflow.push(rectFor(el));
                if (hasOwnText && (html.scrollWidth > html.clientWidth + 2 || html.scrollHeight > html.clientHeight + 2 || st.textOverflow === "ellipsis" || st.webkitLineClamp))
                    clippedText.push(rectFor(el));
                if ((st.position === "fixed" || st.position === "sticky") && r.width > viewport.width * 0.7 && r.height > Math.min(140, viewport.height * 0.25))
                    stickyObstructions.push(rectFor(el));
                if ((el.matches("table,[role='table'],[role='grid']") || el.querySelector("table,[role='grid']")) && html.scrollWidth > viewport.width + 2 && !el.closest("[style*='overflow'],.table-responsive,[class*='scroll']"))
                    tableFailures.push(rectFor(el));
                if (el.matches("[role='dialog'],dialog,.modal,[aria-modal='true']") && (r.right > viewport.width || r.bottom > viewport.height || r.left < 0 || r.top < 0))
                    modalFit.push(rectFor(el));
                if ((st.overflowY === "auto" || st.overflowY === "scroll") && html.scrollHeight > html.clientHeight + 80 && r.height > viewport.height * 0.65)
                    scrollTraps.push(rectFor(el));
            });
            const interactives = Array.from(document.querySelectorAll("a[href],button,input,select,textarea,[role='button'],[role='link'],[tabindex]:not([tabindex='-1'])")).filter(visible);
            for (const el of interactives.slice(0, 70)) {
                ...
                if (!name && el.matches("button,a,[role='button'],[role='link']"))
                    missingFunctionalLabels.push(selectorFor(el));
                try {
        ...
        if (zoomData.overflow.length || zoomData.requiresHScroll) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.overflow[0]?.selector || "body", `Issue-specific reflow evidence captured at ${zoomViewport.width}x${zoomViewport.height} after closing transient menus/popups. Highlighted element is the first horizontal-overflow offender.`);
            issues.push({
                ...
                selectors: zoomData.overflow.length ? zoomData.overflow.map((item) => item.selector) : ["body"],
                affectedElements: zoomData.overflow.map((item) => `${item.selector} (${item.width}x${item.height}, right=${item.right})`),
        ...
        if (zoomData.clippedText.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.clippedText[0].selector, `Issue-specific clipped-text evidence captured at ${zoomViewport.width}x${zoomViewport.height} after closing transient menus/popups. Highlighted element is the first clipped text container.`);
            issues.push({
                ...
                selectors: zoomData.clippedText.map((item) => item.selector),
                affectedElements: zoomData.clippedText.map((item) => `${item.selector}: "${item.text || ""}"`),
        ...
        if (zoomData.stickyObstructions.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.stickyObstructions[0].selector, `Issue-specific sticky/obstruction evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first excessive fixed/sticky region.`);
            issues.push({
                ...
                selectors: zoomData.stickyObstructions.map((item) => item.selector),
                affectedElements: zoomData.stickyObstructions.map((item) => `${item.selector} (${item.width}x${item.height})`),
        ...
        if (zoomData.modalFit.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.modalFit[0].selector, `Issue-specific dialog-fit evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first dialog/modal that does not fit.`);
            issues.push({
                ...
                selectors: zoomData.modalFit.map((item) => item.selector),
                affectedElements: zoomData.modalFit.map((item) => `${item.selector} bounds ${item.left},${item.top},${item.right},${item.bottom}`),
        ...
        if (zoomData.tableFailures.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.tableFailures[0].selector, `Issue-specific table/grid reflow evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first table/grid offender.`);
            issues.push({
                ...
                selectors: zoomData.tableFailures.map((item) => item.selector),
                affectedElements: zoomData.tableFailures.map((item) => `${item.selector} (${item.width}px wide)`),
        ...
        if (zoomData.focusFailures.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.focusFailures[0].selector, `Issue-specific focus-at-zoom evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first focusable control detected as clipped, obscured, off-screen, or missing visible focus.`);
            issues.push({
                ...
                selectors: zoomData.focusFailures.map((item) => item.selector),
                affectedElements: zoomData.focusFailures.map((item) => `${item.selector} bounds ${item.left},${item.top},${item.right},${item.bottom}`),
        ...
        if (zoomData.crowdedTargets.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.crowdedTargets[0].selector, `Issue-specific crowded-target evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first overlapping/crowded interactive target.`);
            issues.push({
                ...
                selectors: zoomData.crowdedTargets.map((item) => item.selector),
                affectedElements: zoomData.crowdedTargets.map((item) => `${item.selector} (${item.width}x${item.height})`),
        ...
        if (zoomData.missingFunctionalLabels.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.missingFunctionalLabels[0], `Issue-specific missing-label evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first interactive control without a usable label.`);
            issues.push({
        ...
        if (zoomData.scrollTraps.length) {
            const evidence = await screenshotEvidenceForSelector(page, zoomData.scrollTraps[0].selector, `Issue-specific nested-scroll evidence captured at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first large nested scroll region.`);
            issues.push({
                ...
                selectors: zoomData.scrollTraps.map((item) => item.selector),
                affectedElements: zoomData.scrollTraps.map((item) => `${item.selector} (${item.width}x${item.height})`),
        ...
        const expandedData = await page.evaluate(async () => {
            ...
            const triggers = Array.from(document.querySelectorAll("[aria-expanded='false'],[aria-haspopup],summary,button,[role='button']"))
                .filter(visible)
                .slice(0, 12);
            const failures = [];
            for (const trigger of triggers) {
                ...
                const panels = Array.from(document.querySelectorAll("[role='menu'],[role='listbox'],[role='dialog'],[aria-modal='true'],.dropdown-menu,.menu,.popover,.tooltip,[data-popper-placement]"))
                ...
                for (const panel of panels) {
                    ...
                    if (offscreen || detached || clipped)
                        failures.push({ trigger: rectSummary(trigger), panel: rectSummary(panel), offscreen, detached, clipped });
                }
                if (failures.length)
                    break;
                try {
        ...
        if (expandedData.length) {
            const expandedEvidence = await screenshotEvidenceForSelector(page, expandedData[0].panel.selector, `Captured after intentionally opening an expandable control at ${zoomViewport.width}x${zoomViewport.height}; highlighted element is the failing popup/menu surface.`, false);
            issues.push({
                ...
                selectors: expandedData.map((item) => item.panel.selector),
                affectedElements: expandedData.map((item) => `${item.panel.selector} from trigger ${item.trigger.selector}`),
        ...
        const textSpacing = await page.evaluate(() => {
            ...
            const failures = [];
            const selectorFor = (el) => el.id ? `${el.tagName.toLowerCase()}#${CSS.escape(el.id)}` : el.tagName.toLowerCase();
            document.querySelectorAll("p,span,a,button,label,li,td,th,h1,h2,h3,h4,h5,h6,div").forEach((el) => {
                const r = el.getBoundingClientRect();
                const st = getComputedStyle(el);
                const hasText = (el.textContent || "").trim().length > 2;
                if (!hasText || r.width <= 0 || r.height <= 0 || st.display === "none" || st.visibility === "hidden")
                    return;
        ...
        if (textSpacing.length) {
            const spacingEvidence = await screenshotEvidenceForSelector(page, textSpacing[0].selector, `Issue-specific evidence captured while WCAG text-spacing overrides are still applied at ${zoomViewport.width}x${zoomViewport.height}. Highlighted element is the first text-spacing failure.`);
            issues.push({
                ...
                selectors: textSpacing.map((item) => item.selector),
                affectedElements: textSpacing.map((item) => `${item.selector}: "${item.text}"`),
        ...
        if (persistentTransients.length || persistentExpanded.length || viewportMismatch || introducedHorizontalOverflow || overflowLockChanged || transformPersisted) {
            ...
            const evidence = await screenshotEvidenceForSelector(page, selector, `Post-zoom recovery evidence captured after restoring the original ${originalVp.width}x${originalVp.height} viewport. The highlighted element or page state remained inconsistent with the pre-zoom baseline.`, false);
            const reasons = [
        ...
        logger_1.logger.debug("Zoom checks failed:", err);
    }
    finally {
        await restoreInjectedUi(page);
        await page.setViewportSize(originalVp).catch(() => undefined);
...
async function runPointerChecks(page, url, state, phase) {
    const issues = [];
    const smallTargets = await page.evaluate(() => {
        const MIN = 24;
        const out = [];
        document.querySelectorAll("a[href],button,input,select,textarea,[role='button'],[role='link'],[role='checkbox'],[role='radio'],[tabindex]").forEach((el) => {
            const r = el.getBoundingClientRect();
            ...
            if (!visible || disabled || inlineTextLink)
                return;
    ...
    const dragOnly = await page.evaluate(() => {
        ...
        document.querySelectorAll("[draggable='true']").forEach((el) => {
            ...
            if (!hasBtnAlternative)
                out.push(el.id ? `${el.tagName.toLowerCase()}[draggable]#${CSS.escape(el.id)}` : `${el.tagName.toLowerCase()}[draggable]`);
        });
    ...
    const gestureOnly = await page.evaluate(() => {
        const out = [];
        document.querySelectorAll("[class*='swipe'],[class*='gesture'],[data-swipe],[data-gesture],[class*='carousel'],[class*='slider']").forEach((el) => {
            const hasArrows = el.querySelector("button,[role='button'],[aria-label*='next' i],[aria-label*='prev' i]");
            if (!hasArrows)
                out.push(el.className ? `.${String(el.className).split(" ")[0]}` : el.tagName.toLowerCase());
        });
        return out.slice(0, 20);
    }).catch(() => []);

...
exports.runScreenReader = runScreenReader;
 ...
 */
async function runScreenReader(page, url, stateLabel, phase = "initial", includeRawTree = false) {
    const started = Date.now();
    try {
        logger_1.logger.info(`[sr] Capturing AX tree for ${url}`);
        const nodes = await extractAxTree(page);
        logger_1.logger.info(`[sr]   nodes=${nodes.length} interactive=${countInteractive(nodes)} landmarks=${nodes.filter(n => LANDMARK_ROLES.has(n.role)).length}`);
        ...
        logger_1.logger.info(`[sr]   issues=${issues.length} score=${score} in ${Date.now() - started}ms`);
        return { issues, report };
    }
    catch (err) {
        logger_1.logger.warn(`[sr] Screen reader analysis failed for ${url}: ${err?.message || err}`);
        return {
            issues: [],
            report: {
                url,
...
// -----------------------------------------------------------------------------
async function extractAxTree(page) {
    const cdp = await page.context().newCDPSession(page);
    try {
        await cdp.send("Accessibility.enable");
        const { nodes: rawNodes } = await cdp.send("Accessibility.getFullAXTree");
        return rawNodes.map(mapAxNode).filter(Boolean);
    }
    finally {
        try {
...
function mapAxNode(raw) {
    ...
    if (levelProp?.value?.value != null)
        level = Number(levelProp.value.value);
    return {
 ...
 */
function correlateDomWithAxTree(nodes, domEntries) {
...
// -----------------------------------------------------------------------------
function extractLandmarks(nodes) {
    return nodes
...
async function extractLiveRegions(page) {
    return await page.evaluate(() => {
        const results = [];
...
function buildAnnouncementTranscript(nodes) {
    const transcript = [];
    let index = 0;
    for (const node of nodes) {
        if (node.ignored)
            continue;
        if (!INTERACTIVE_ROLES.has(node.role) && node.role !== "heading" && !LANDMARK_ROLES.has(node.role))
            continue;
...
function countReadingOrderDivergences(nodes) {
    ...
    for (const node of nodes) {
        ...
        if (!node.boundingBox)
            continue;
        if (!INTERACTIVE_ROLES.has(node.role) && node.role !== "heading")
            continue;
...
function countInteractive(nodes) {
    return nodes.filter(n => !n.ignored && INTERACTIVE_ROLES.has(n.role)).length;
}
// -----------------------------------------------------------------------------
// Issue derivation
// -----------------------------------------------------------------------------
function makeIssue(ruleId, severity, message, url, category, wcag, node, stateLabel, phase) {
    return {
        ruleId,
        severity,
        category,
...
function deriveIssues(nodes, landmarks, headings, transcript, url, stateLabel, phase) {
    const issues = [];
    ...
    for (const node of nodes) {
        ...
        if (node.name.trim().length === 0) {
            issues.push(makeIssue("sr-missing-name", "critical", `Interactive element (role="${node.role}") has no accessible name. A screen reader will announce this only as "${node.role}" with no context, leaving the user unable to know what activating it will do.`, url, "screen-reader", ["4.1.2"], node, stateLabel, phase));
        }
    }
    ...
    for (const node of nodes) {
        ...
        if (nm && GENERIC_NAMES.has(nm)) {
            issues.push(makeIssue("sr-generic-name", "moderate", `Accessible name "${node.name}" is too generic. Screen reader users navigating by pulling up a list of ${node.role}s will see multiple identical entries. Prefer a name that describes the destination or action (e.g. "Read our privacy policy" instead of "Read more").`, url, "screen-reader", ["2.4.4"], node, stateLabel, phase));
        }
    }
    ...
    for (const [key, list] of byRoleName.entries()) {
        if (list.length >= 3) {
            ...
            issues.push(makeIssue("sr-duplicate-name", "moderate", `${list.length} ${list[0].role} elements share the accessible name "${name}". Screen reader users navigating by ${list[0].role} list will be unable to distinguish them.`, url, "screen-reader", ["2.4.4"], list[0], stateLabel, phase));
        }
    }
    // 4. Missing main landmark.
    if (!landmarks.some(l => l.role === "main")) {
        issues.push(makeIssue("sr-no-main", "moderate", `Page has no <main> landmark. Screen reader users use landmarks to skip navigation and jump to primary content; without a main landmark this shortcut is unavailable.`, url, "screen-reader", ["1.3.1", "2.4.1"], undefined, stateLabel, phase));
    }
    // 5. Heading structure.
    const h1Count = headings.filter(h => h.level === 1).length;
    if (headings.length > 0 && h1Count === 0) {
        issues.push(makeIssue("sr-no-h1", "moderate", `Page has ${headings.length} heading(s) but no h1. Screen reader users often press "1" (NVDA) or use rotor (VoiceOver) to jump to the primary heading; without an h1 they cannot orient.`, url, "screen-reader", ["1.3.1", "2.4.6"], undefined, stateLabel, phase));
    }
    // Skipped heading levels
    let previousLevel = 0;
    for (const h of headings) {
        if (previousLevel > 0 && h.level > previousLevel + 1) {
            issues.push(makeIssue("sr-heading-skip", "moderate", `Heading level jumps from h${previousLevel} to h${h.level} ("${h.text}"). Screen reader users navigating by heading level expect no gaps; skipping levels breaks their mental model of the page hierarchy.`, url, "screen-reader", ["1.3.1"], { id: "", role: "heading", name: h.text, states: [], ignored: false, childIds: [], level: h.level, domSelector: h.domSelector }, stateLabel, phase));
        }
    ...
    const divergences = countReadingOrderDivergences(nodes);
    if (divergences >= 3) {
        issues.push(makeIssue("sr-reading-order", "moderate", `Detected ${divergences} places where DOM order differs from visual (Y-coordinate) order by more than 100px. Screen readers announce in DOM order; when the visual layout uses flex/grid/absolute positioning to reorder, SR users experience content in a different sequence than sighted users. This can make instructions and cause-effect relationships confusing.`, url, "screen-reader", ["1.3.2"], undefined, stateLabel, phase));
    }
    return issues;
}
function computeScore(issues, nodeCount) {
    if (nodeCount === 0)
        return 0;
    const weights = {
        critical: 20,

...
function affectedElementLabels(issue) {
...
function cleanEvidenceText(value) {
    return String(value || "")
        .replace(/Use this evidence together with the selector, HTML snippet, issue message, and recommended fix\./gi, "Use this evidence together with the affected component, issue message, and recommended fix.")
        .trim();
}
function friendlyElementName(selector) {
    if (!selector)
        return "Page element";
...
function issueTitle(issue) {
    ...
    if (/text.*clip|truncat/i.test(message))
        return "Text content may be clipped or truncated";
    if (/meta-viewport/i.test(rule))
        return "Mobile zoom is restricted";
    if (/label|input-no-label/i.test(rule))
        return "Form control is missing a clear label";
    if (/color:focus-indicator-low-contrast/i.test(rule))
        return "Focus indicator contrast is too low";
    if (/color:contrast-insufficient/i.test(rule))
        return "Text contrast is too low";
    if (/color|contrast/i.test(rule))
        return "Text or control contrast is too low";
    if (/focus:invisible/i.test(rule))
        return "Keyboard focus indicator is not visible";
    if (/focus:trap|trap-missing/i.test(rule))
        return "Keyboard focus can become trapped or unusable";
    if (/aria-required-children/i.test(rule))
        return "ARIA role is missing required child elements";
    if (/landmark.*unique|landmark-unique/i.test(rule))
        return "Landmark needs a unique accessible name";
    if (/nested-interactive/i.test(rule))
        return "Interactive controls are nested";
    if (/target-size/i.test(rule))
        return "Interactive target is too small";
    if (/heading-order/i.test(rule))
        return "Heading order is not logical";
    return message || rule || "Accessibility issue";
}
function conciseImpact(issue) {
    const rule = `${issue.rule_id || ""} ${issue.message || ""}`;
    if (/clip|truncat|overflow/i.test(rule))
        return "Important text may be hidden, especially at zoom or smaller viewport sizes.";
    if (/focus/i.test(rule))
        return "Keyboard users may lose their place or be unable to operate the page predictably.";
    if (/meta-viewport/i.test(rule))
        return "Mobile users may be blocked from zooming content.";
    if (/aria|role|landmark/i.test(rule))
        return "Screen reader users may receive confusing structure, labels, or roles.";
    if (/label/i.test(rule))
        return "Users may not understand what a form control is asking for.";
    if (/color|contrast/i.test(rule))
        return "Low-vision users may not be able to read the affected content.";
    if (/target-size|pointer/i.test(rule))
        return "Users may have difficulty selecting or tapping the control.";
    return "This can make the page harder to understand, navigate, or operate.";
}
function verifyStep(issue) {
    const rule = `${issue.rule_id || ""} ${issue.message || ""}`;
    if (/clip|truncat|overflow/i.test(rule))
        return "Zoom to 200%, resize the viewport, and confirm the full text remains visible or discoverable.";
    if (/focus/i.test(rule))
        return "Use Tab and Shift+Tab and confirm visible focus on the affected control.";
    if (/meta-viewport/i.test(rule))
        return "Use mobile emulation and confirm browser/pinch zoom is allowed.";
    if (/aria|role|landmark/i.test(rule))
        return "Inspect with a screen reader or accessibility tree and confirm the correct name, role, and structure.";
    if (/label/i.test(rule))
        return "Focus the control and confirm a clear visible label or accessible name is announced.";
    if (/color|contrast/i.test(rule))
        return "Check the foreground/background colors against WCAG contrast thresholds.";
    return "Open the listed page, reproduce the issue, apply the fix, and re-run the scan.";
}
function recommendedFix(issue) {
    const suggestion = String(issue.fix_suggestion || "").trim();
    if (suggestion && !/^fix any of the following/i.test(suggestion))
        return suggestion;
    const rule = `${issue.rule_id || ""} ${issue.message || ""}`;
    if (/clip|truncat|overflow/i.test(rule))
        return "Allow content to wrap, increase container height, or provide an accessible expansion/full-text pattern.";
    if (/focus/i.test(rule))
        return "Add a visible focus style that is not hidden by overlays, clipping, or color-only changes.";
    if (/meta-viewport/i.test(rule))
        return "Remove maximum-scale and user-scalable restrictions from the viewport meta tag.";
    if (/label/i.test(rule))
        return "Associate each control with a visible label, aria-label, or aria-labelledby value.";
    if (/aria-required-children/i.test(rule))
        return "Use the required child roles or replace the custom ARIA pattern with semantic HTML.";
    if (/landmark/i.test(rule))
        return "Give repeated landmarks unique names, for example with aria-label or aria-labelledby.";
    if (/nested-interactive/i.test(rule))
        return "Do not place a link, button, or input inside another interactive control.";
    if (/color|contrast/i.test(rule))
        return "Adjust the text/icon and background colors to meet the applicable WCAG contrast ratio.";
...
function wcagLevel(issue) {
    ...
    for (const raw of rawTags) {
        ...
        if (/^wcag\d*aaa$/.test(lower))
            found.add("AAA");
        else if (/^wcag\d*aa$/.test(lower))
            found.add("AA");
        else if (/^wcag\d*a$/.test(lower))
            found.add("A");
    }
    if (found.has("A"))
        return "A";
    ...
    if (found.has("AAA"))
        return "AAA";
    if (rawTags.some(tag => tag.toLowerCase() === "best-practice"))
        return "Advisory";
    if (String(issue.category || "").toLowerCase() === "advisory")
        return "Advisory";
    return "Needs review";
}
const wcagCriterionLevels = {
...
function wcagCriterionFromTag(item) {
    const value = String(item || "").toLowerCase().replace(/^wcag\s*/i, "").trim();
    if (!value || /^[\d.]*a+$/.test(value))
        return null;
...
function wcagTextWithLevel(item) {
...
function wcagText(issue) {
    const wcag = Array.from(new Set(asArray(issue.wcag_criteria).concat(asArray(issue.tags)).map((item) => wcagTextWithLevel(String(item))).filter(Boolean))).slice(0, 3);
...
function prettyRuleName(ruleId) {
...
function testCaseDisplayName(tc, linkedIssue) {
    const raw = String(tc.name || "").trim();
    if (linkedIssue)
        return `${prettyRuleName(linkedIssue.rule_id)} Check`;
    const ruleMatch = `${tc.name || ""} ${tc.description || ""}`.match(/\b((?:axe|heuristic|keyboard|focus|pointer|zoom|color):[a-z0-9-]+)\b/i);
    if (ruleMatch)
        return `${prettyRuleName(ruleMatch[1])} Check`;
    return raw
        .replace(/^\[(MANUAL|HYBRID|CRITICAL|SERIOUS|MODERATE|MINOR)\]\s*/i, "")
        .replace(/^Automated check\s*:?\s*/i, "")
        .trim() || "Accessibility Verification";
}
function testCaseSummary(tc, linkedIssue) {
    if (linkedIssue)
        return `Verify that ${issueTitle(linkedIssue).toLowerCase()} is resolved for the affected page.`;
    const cleaned = removeUrls(String(tc.description || ""));
    if (/screen reader/i.test(`${tc.name} ${tc.description}`))
        return "Validate reading order, announcements, names, roles, and state changes with assistive technology.";
    if (/keyboard-only|keyboard validation/i.test(`${tc.name} ${tc.description}`))
        return "Validate the main task flow using keyboard-only navigation.";
    if (/dynamic|menus|modals|accordions|tabs/i.test(`${tc.name} ${tc.description}`))
        return "Validate task-critical interactive states such as dialogs, menus, accordions, tabs, and validation states.";
    if (/responsive|zoom|touch/i.test(`${tc.name} ${tc.description}`))
        return "Validate zoom, reflow, mobile viewport behavior, touch targets, and orientation support.";
    if (/form completion|error recovery|form validation/i.test(`${tc.name} ${tc.description}`))
        return "Validate form submission, error identification, recovery guidance, autocomplete, and success messaging.";
    return cleaned || "Review the listed steps and mark the case after verification.";
}
function isProcedureStep(step) {
    return step && typeof step === "object" && "stepText" in step && "coverageType" in step;
}
function coverageColor(type) {
    return type === "automated" ? "#047857" : type === "hybrid" ? "#7c3aed" : "#64748b";
}
function procedureCoverageHtml(steps) {
    const mapped = steps.filter(isProcedureStep);
    if (!mapped.length)
        return "";
    const counts = mapped.reduce((acc, step) => {
    ...
    return `${badges}<div class="procedure-steps">${mapped.map((step, index) => `
...
const REPORT_SECTIONS = ["executive", "navigation", "interactions", "testcases", "states", "issues"];
...
function hasReportSection(sections, section) {
    return sections.has(section);
}
async function generateScanReport(scanId, requestedSections) {
    ...
    const [scanResult, issuesResult, testCasesResult, snapshotResult] = await Promise.all([
        db_1.db.query("SELECT s.*, u.full_name as created_by_name FROM scans s JOIN users u ON u.id = s.created_by WHERE s.id = $1", [scanId]),
        db_1.db.query(`SELECT * FROM issues WHERE scan_id = $1 AND COALESCE(false_positive, false) = false
      ...
      CASE severity WHEN 'critical' THEN 1 WHEN 'serious' THEN 2 WHEN 'moderate' THEN 3 ELSE 4 END, created_at`, [scanId]),
        db_1.db.query("SELECT * FROM test_cases WHERE scan_id = $1 ORDER BY status, created_at", [scanId]),
        db_1.db.query("SELECT url, phase, a11y_tree, created_at FROM dom_snapshots WHERE scan_id = $1 ORDER BY created_at, id", [scanId]),
    ]);
    ...
    const navigatedUrls = [...new Set((persistedNavigationUrls.length ? persistedNavigationUrls : [...urls, ...snapshots.map((snapshot) => snapshot.url), ...issues.map((issue) => issue.url)]).map(String).map(url => url.trim()).filter(Boolean))];
    ...
    for (const issue of unresolvedIssues) {
        ...
        if (!key) {
            singletons.push(issue);
            continue;
        }
        if (!landmarkGroups.has(key))
            landmarkGroups.set(key, []);
    ...
    const sortedIssues = [...enrichedIssues].sort((a, b) => (a.priority || 5) - (b.priority || 5) || severityRank(a.severity) - severityRank(b.severity));
    const sevCounts = { critical: 0, serious: 0, moderate: 0, minor: 0 };
    const levelCounts = { A: 0, AA: 0, AAA: 0, Advisory: 0, "Needs review": 0 };
    for (const issue of unresolvedIssues) {
    ...
    const inProgressCount = testCases.filter((tc) => tc.status === "pending").length;
    const testTotal = testCases.length || 0;
    const pieGradient = (items) => {
    ...
    const pieLegend = (items, total) => items
    ...
    const categoryCounts = unresolvedIssues.reduce((acc, issue) => {
    ...
    const principleForIssue = (issue) => {
        ...
        for (const raw of rawTags) {
            ...
            if (first === "1")
                principles.add("Perceivable");
            else if (first === "2")
                principles.add("Operable");
            else if (first === "3")
                principles.add("Understandable");
            else if (first === "4")
                principles.add("Robust");
        }
        if (principles.has("Perceivable"))
            return "Perceivable";
        if (principles.has("Operable"))
            return "Operable";
        if (principles.has("Understandable"))
            return "Understandable";
        if (principles.has("Robust"))
            return "Robust";
        const catRule = `${issue.category || ""} ${issue.rule_id || ""}`.toLowerCase();
        if (/keyboard|focus|pointer|target|zoom|reflow/.test(catRule))
            return "Operable";
        if (/label|error|input|form/.test(catRule))
            return "Understandable";
        if (/aria|role|name/.test(catRule))
            return "Robust";
        return "Needs review";
    };
    const principleCounts = unresolvedIssues.reduce((acc, issue) => {
    ...
    const severityOptions = ["critical", "serious", "moderate", "minor"]
        ...
        .join("");
    const priorityOptions = [...new Set(sortedIssues.map((issue) => Number(issue.priority || 5)))]
        .sort((a, b) => a - b)
    ...
    const caseRows = testCases.map((tc, index) => {
        ...
        const linkedIssue = issues.find((issue) => issue.id === tc.issue_id);
    ...
    const caseStatusOptions = ["pass", "fail", "pending"].filter(status => testCases.some((tc) => (tc.status === status || (status === "pending" && tc.status !== "pass" && tc.status !== "fail")))).map(status => `<label><input type="checkbox" data-case-filter="status" value="${status}"> ${status === "pass" ? "Pass" : status === "fail" ? "Fail" : "In progress"}</label>`).join("");
    const caseTypeOptions = ["automated", "manual", "hybrid"].filter(type => testCases.some((tc) => { const text = `${tc.category || ""} ${tc.status || ""}`.toLowerCase(); const actual = text.includes("hybrid") ? "hybrid" : text.includes("manual") ? "manual" : "automated"; return actual === type; })).map(type => `<label><input type="checkbox" data-case-filter="type" value="${type}"> ${type[0].toUpperCase()}${type.slice(1)}</label>`).join("");
    ...
    const stateBreakdown = issues.reduce((acc, issue) => {
        ...
        acc[state] || (acc[state] = { total: 0, critical: 0, serious: 0, moderate: 0, minor: 0, screenshots: 0 });
        acc[state].total += 1;
        acc[state][issue.severity || "minor"] = (acc[state][issue.severity || "minor"] || 0) + 1;
        if (issue.evidence_screenshot)
            acc[state].screenshots += 1;
        return acc;
    }, {});
    configuredStates.forEach(state => { stateBreakdown[state] || (stateBreakdown[state] = { total: 0, critical: 0, serious: 0, moderate: 0, minor: 0, screenshots: 0 }); });
    const stateEntries = Object.entries(stateBreakdown).sort((a, b) => b[1].total - a[1].total || a[0].localeCompare(b[0]));
    const stateColors = ["#7c3aed", "#e11d48", "#ff9f43", "#0f766e", "#0891b2", "#a78bfa", "#64748b", "#ffd60a", "#f97316", "#14b8a6"];
    const stateItems = stateEntries.map(([state, row], index) => ({ label: state, count: Number(row.total || 0), color: stateColors[index % stateColors.length] }));
    ...
    const maxStateIssues = Math.max(1, ...Object.values(stateBreakdown).map((row) => Number(row.total || 0)));
    const stateRows = stateEntries.map(([state, row], index) => {
    ...
    const snapshotTimeline = persistedNavigationUrls.length ? persistedNavigationUrls.map((url, index) => ({
        url,
    ...
    })) : snapshots.length ? snapshots : urls.map((url) => ({
        url,
        phase: "configured target",
        created_at: scan.started_at || scan.created_at || new Date().toISOString()
    }));
    const navigationRows = snapshotTimeline.map((item, index) => {
    ...
    const interactionReports = snapshots
        .map((snapshot) => ({ snapshot, tree: snapshot.a11y_tree && typeof snapshot.a11y_tree === "object" ? snapshot.a11y_tree : {} }))
        .filter((entry) => entry.tree?.type === "controlled-interaction-report");
    const interactionRows = interactionReports.flatMap((entry) => {
        const mode = entry.tree.mode || "safe-auto";
        return asArray(entry.tree.items).map((item) => `
      <tr>
    ...
    const interactionSummary = interactionReports.reduce((acc, entry) => {
    ...
    const contentOptions = [
        ...
        ["states", "UI states"],
        ["issues", "Issues"],
    ].map(([value, label]) => `<label><input type="checkbox" data-section-filter value="${value}" ${selectedSections.has(value) ? "checked" : ""}> ${label}</label>`).join("");
    ...
    <div class="report">
      <header>
        <div class="topline">
          ...
          <div class="meta">
            <div>Generated ${escapeHtml((0, date_fns_1.format)(new Date(), "MMM d, yyyy HH:mm"))}</div>
            ${completedAt ? `<div>Completed ${escapeHtml((0, date_fns_1.format)(completedAt, "MMM d, yyyy HH:mm"))}</div>` : ""}

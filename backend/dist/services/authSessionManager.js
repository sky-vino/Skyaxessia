...
const scanQueue_1 = require("./scanQueue");
// -----------------------------------------------------------------------------
// Config
// -----------------------------------------------------------------------------
...
async function startSession(input) {
    if (sessions.size >= MAX_CONCURRENT) {
        throw new Error(`This server is currently holding ${sessions.size} concurrent auth session(s), which is the configured cap of ${MAX_CONCURRENT}. Please wait ~30 seconds and try again, or cancel an existing pending session.`);
    }
    const now = Date.now();
    const id = (0, crypto_1.randomUUID)();
    const otpChannel = input.otpChannel || "email";
    const session = {
        id,
        phase: "launching",
        createdAt: now,
    ...
    logger_1.logger.info(`[auth-session ${id}] Launching for ${input.targetUrl}, channel=${otpChannel}`);
    ...
    void driveToOtpScreen(session, input).catch(err => {
        logger_1.logger.error(`[auth-session ${id}] driveToOtpScreen failed:`, err);
...
async function submitOtp(id, otp) {
    const session = sessions.get(id);
    if (!session)
    ...
    logger_1.logger.info(`[auth-session ${id}] Submitting OTP (${otp.length} digits)`);
    try {
        ...
        logger_1.logger.info(`[auth-session ${id}] Auth succeeded, ${cookies.length} cookies extracted`);
        ...
        const scanInsert = await db_1.db.query(`INSERT INTO scans (name, urls, project_id, created_by, state_label, auth_config, scan_options, status)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'queued') RETURNING *`, [
        ...
        logger_1.logger.info(`[auth-session ${id}] Scan ${scan.id} queued`);
        ...
        logger_1.logger.warn(`[auth-session ${id}] OTP submit failed: ${err?.message || err}`);
        session.phase = "failed";
...
async function cancelSession(id) {
    ...
    await cleanupBrowser(session);
    sessions.delete(id);
    logger_1.logger.info(`[auth-session ${id}] Cancelled by user`);
}
...
// Internal flow
// -----------------------------------------------------------------------------
async function driveToOtpScreen(session, input) {
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Navigating to ${input.targetUrl}`);
    try {
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Attempting to dismiss cookie consent...`);
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Cookie consent phase done. Current URL: ${page.url()}`);
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Auto-detecting login form...`);
    ...
    if (!form || !form.usernameFound || !form.passwordFound) {
        ...
        throw new Error(`Could not find a login form on ${page.url()}. ` +
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Form detected: ${form.description}`);
    logger_1.logger.info(`[auth-session ${session.id}] Filling username`);
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Filling password`);
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Clicking submit button`);
    const submitted = await clickDetectedSubmit(page);
    if (!submitted) {
        logger_1.logger.info(`[auth-session ${session.id}] No submit button found; pressing Enter as fallback`);
    ...
    if (session.otpChannel === "email") {
        logger_1.logger.info(`[auth-session ${session.id}] Clicking "Send by email"`);
        await clickDeliveryChannel(page, "email");
    }
    else if (session.otpChannel === "sms") {
        logger_1.logger.info(`[auth-session ${session.id}] Clicking "Send via SMS"`);
    ...
    logger_1.logger.info(`[auth-session ${session.id}] OTP sent to: ${session.otpMaskedRecipient || "<unknown recipient>"}`);
    session.pendingScan = {
        scanName: input.scanName,
    ...
    logger_1.logger.info(`[auth-session ${session.id}] Ready — waiting for user to enter OTP`);
}
async function findLoginForm(page) {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        last = await page.evaluate(() => {
            ...
            const inputs = all.filter(el => el.tagName === "INPUT" && isVisible(el));
            ...
            const scoreUsername = (el) => {
                ...
                if (type === "text" || type === "")
                    score += 3;
                if (/user|email|mail|login|account/.test(combined))
                    score += 4;
            ...
            const buttons = all.filter(el => (el.tagName === "BUTTON" || (el.tagName === "INPUT" && ["submit", "button"].includes(el.type))) &&
                isVisible(el));
            const submit = buttons.find(b => b.type === "submit") ||
                buttons.find(b => /accedi|sign\s?in|log\s?in|entra|conferma/i.test((b.innerText || b.value || "").trim())) ||
                buttons[0] || null;
            // Mark for later retrieval - we can't return DOM refs from page.evaluate.
            if (username)
                username.dataset.axessiaRole = "username";
            if (password)
                password.dataset.axessiaRole = "password";
            if (submit)
                submit.dataset.axessiaRole = "submit";
            const desc = [
                ...
                submit ? `submit=<${submit.tagName.toLowerCase()} text="${(submit.innerText || submit.value || "").trim().slice(0, 40)}">` : "submit=NONE",
            ].join(", ");
            return {
...
async function fillDetectedField(page, role, value) {
    ...
    return await page.evaluate(({ role, value }) => {
        ...
        const all = [];
        walkAll(document, all);
        const el = all.find(e => e.dataset?.axessiaRole === role);
        if (!el)
            return false;
        try {
            el.focus();
...
async function clickDetectedSubmit(page) {
    return await page.evaluate(() => {
        ...
        const all = [];
        walkAll(document, all);
        const btn = all.find(e => e.dataset?.axessiaRole === "submit");
        if (!btn)
            return false;
        try {
            btn.click();
...
async function saveDiagnosticSnapshot(page, sessionId, label) {
    try {
        ...
        logger_1.logger.info(`[auth-session ${sessionId}] Diagnostic snapshot: ${p}`);
        logger_1.logger.info(`[auth-session ${sessionId}]   URL:   ${url}`);
        logger_1.logger.info(`[auth-session ${sessionId}]   Title: ${title}`);
        ...
        logger_1.logger.info(`[auth-session ${sessionId}]   Visible inputs on page: ${JSON.stringify(inputs).slice(0, 400)}`);
        return p;
    }
    catch {
        return undefined;
...
async function launchStealthChromium() {
...
function resolveFullChromiumPath() {
...
async function dismissCookieConsent(page) {
    ...
    for (const frame of frames) {
        for (const sel of acceptSelectors) {
            try {
                ...
                if (!visible)
                    continue;
                logger_1.logger.info(`[auth-session] Cookie banner: found "${sel}", clicking`);
                ...
                if (!stillPresent) {
                    logger_1.logger.info(`[auth-session] Cookie banner dismissed via "${sel}"`);
                    return; // Success
                }
            }
            catch { /* try next */ }
    ...
    logger_1.logger.info("[auth-session] Cookie banner: no selector matched, trying DOM walker fallback");
    const walkerClicked = await page.evaluate(() => {
        ...
        const candidates = all.filter(el => {
            ...
            if (tag === "button" || tag === "a" || role === "button")
                return true;
            if (tag === "input" && ["button", "submit"].includes(el.type))
                return true;
            if (el.classList?.contains("accbtn"))
                return true;
            return false;
        });
        // Match by innerText OR aria-label
        const target = candidates.find(c => {
            const text = (c.innerText || c.value || "").trim();
    ...
    if (walkerClicked) {
        await page.waitForTimeout(700);
        logger_1.logger.info("[auth-session] Cookie banner dismissed via DOM walker fallback");
        return;
    }
    logger_1.logger.warn("[auth-session] Could not dismiss cookie banner via any strategy. Login form may be blocked; will still try to find it.");
}
async function robustFill(page, selectorSpec, value) {
...
async function robustClick(page, selectorSpec) {
    const selectors = selectorSpec.split(" ").filter(Boolean);
    for (const sel of selectors) {
        try {
            if (sel.startsWith("js=")) {
...
async function waitForSecurityPage(page, timeoutMs) {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            if (errorText)
                throw new Error(`Sky returned a block/error page after Accedi: ${errorText.slice(0, 200)}`);
        }
        catch (err) {
            if (err?.message?.startsWith("Sky returned"))
                throw err;
...
async function clickDeliveryChannel(page, channel) {
    ...
    if (alreadyOnTarget.test(bodyText)) {
        logger_1.logger.info(`[auth-session] Page already on ${channel} channel; no switch needed.`);
        return;
    }
    const frames = page.frames();
    for (const frame of frames) {
        for (const text of targetTexts) {
            try {
                ...
                logger_1.logger.info(`[auth-session] Found delivery-channel text "${text}"; clicking`);
                // Try normal click first; if intercepted, force-click.
                try {
                    await el.click({ timeout: 3000 });
                }
                catch (clickErr) {
                    logger_1.logger.info(`[auth-session] Normal click failed (${clickErr?.message?.slice(0, 60)}), trying force-click`);
                ...
                if (alreadyOnTarget.test(newBodyText)) {
                    logger_1.logger.info(`[auth-session] Verified: page now shows ${channel} channel.`);
                    return;
                }
                ...
                if (recipientLooksRight) {
                    logger_1.logger.info(`[auth-session] Verified: recipient info now shows ${channel} format.`);
                    return;
                }
                logger_1.logger.warn(`[auth-session] Clicked "${text}" but page still does not appear to be on ${channel} channel. Body text sample: "${newBodyText.slice(0, 200)}"`);
                // Don't return - try the next variant / frame in case we misidentified.
            }
            catch (err) {
                logger_1.logger.info(`[auth-session] Attempt with text "${text}" errored: ${err?.message?.slice(0, 80)}`);
            }
        }
    }
    ...
    logger_1.logger.info(`[auth-session] Playwright locators exhausted for ${channel}; trying DOM walker fallback`);
    ...
    const walkerClicked = await page.evaluate((pattern) => {
        ...
        if (direct) {
            try {
                direct.click();
                return { hit: true, via: "direct", text: (direct.innerText || "").slice(0, 80) };
            }
            catch {
                return { hit: false, via: "direct-click-threw", text: "" };
            }
        }
        ...
        if (anyMatch) {
            ...
            if (clickable) {
                try {
                    clickable.click();
                    return { hit: true, via: "ancestor", text: (clickable.innerText || "").slice(0, 80) };
                }
                catch {
                    return { hit: false, via: "ancestor-click-threw", text: "" };
                }
            }
            // Last resort: click the matching element itself.
            try {
                anyMatch.click();
                return { hit: true, via: "self", text: anyMatch.innerText.slice(0, 80) };
            }
            catch { /* fall through */ }
        }
    ...
    if (walkerClicked.hit) {
        logger_1.logger.info(`[auth-session] DOM walker clicked ${channel} channel via ${walkerClicked.via}: "${walkerClicked.text}"`);
        await page.waitForLoadState("networkidle", { timeout: 5000 }).catch(() => undefined);
        await page.waitForTimeout(2000);
        const newBodyText = await page.evaluate(() => document.body?.innerText || "").catch(() => "");
        if (alreadyOnTarget.test(newBodyText)) {
            logger_1.logger.info(`[auth-session] Verified after walker click: page now on ${channel}`);
            return;
        }
        const recipientLooksRight = channel === "email"
            ? /EMAIL\s*:\s*[^\s]+@[^\s]+/i.test(newBodyText)
            : /PHONE\s*:\s*[\+\d]/i.test(newBodyText) || /TELEFONO\s*:\s*[\+\d]/i.test(newBodyText);
        if (recipientLooksRight) {
            logger_1.logger.info(`[auth-session] Verified after walker click: recipient now shows ${channel}`);
            return;
        }
        logger_1.logger.warn(`[auth-session] Walker clicked ${channel} target but verification failed. Body: "${newBodyText.slice(0, 300)}"`);
    ...
    logger_1.logger.warn(`[auth-session] Could not switch delivery to "${channel}" on any frame. Walker result: ${walkerClicked.via}. Sky will use its default. Body sample: "${finalBody.slice(0, 500)}"`);
}
async function extractMaskedRecipient(page) {
    return await page.evaluate(() => {
        const body = document.body?.innerText || "";
...
async function fillOtpBoxes(page, otp) {
    ...
    const found = await page.evaluate(() => {
        ...
        const inputs = all.filter(el => el.tagName === "INPUT" &&
            isVisible(el) &&
            (el.maxLength === 1 ||
                /otp|code|codice|sicurezza/i.test(el.className || "") ||
                /otp|code/i.test(el.id || "")));
        // Tag them for retrieval by index.
        inputs.forEach((el, i) => { el.dataset.axessiaOtpIndex = String(i); });
        return {
            count: inputs.length,
    ...
    }).catch(() => ({ count: 0, sampleAttrs: [] }));
    logger_1.logger.info(`[auth-session] fillOtpBoxes: found ${found.count} OTP input candidates`);
    if (found.count === 0) {
        logger_1.logger.info(`[auth-session] fillOtpBoxes: sample DOM state: ${JSON.stringify(found.sampleAttrs).slice(0, 300)}`);
        // Log a broader input inventory so we can see what IS on the page.
        const allInputs = await page.evaluate(() => {
            const out = [];
            function walkAll(root, out2) {
                const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT);
                let n = walker.currentNode;
                while (n) {
                    if (n.nodeType === 1) {
        ...
        logger_1.logger.info(`[auth-session] fillOtpBoxes: full input inventory: ${JSON.stringify(allInputs).slice(0, 500)}`);
        throw new Error(`Only found 0 OTP boxes, expected ${otp.length}. ` +
    ...
    for (let i = 0; i < otp.length; i++) {
        ...
        const ok = await page.evaluate(({ index, value }) => {
            ...
            const all = [];
            walkAll(document, all);
            const el = all.find(e => e.dataset?.axessiaOtpIndex === String(index));
            if (!el)
                return false;
            try {
                el.focus();
        ...
        if (!ok) {
            logger_1.logger.warn(`[auth-session] fillOtpBoxes: digit ${i + 1} of ${otp.length} did not stick`);
        }
        await page.waitForTimeout(60);
    }
    logger_1.logger.info(`[auth-session] fillOtpBoxes: all ${otp.length} digits entered`);
}
async function clickConferma(page) {
    ...
    const clicked = await page.evaluate(() => {
        ...
        const conferma = buttons.find(b => /conferma|he confirms|confirm|verify|verifica/i.test((b.innerText || b.value || "").trim()));
        const target = conferma || buttons.find(b => b.type === "submit");
        if (!target)
            return false;
        try {
            target.click();
    ...
    if (!clicked) {
        logger_1.logger.info("[auth-session] clickConferma: no button found by walker, pressing Enter as fallback");
        await page.keyboard.press("Enter").catch(() => undefined);
    }
    else {
        logger_1.logger.info("[auth-session] clickConferma: clicked Conferma button");
    }
}
async function waitForAuthenticatedLanding(page, targetUrl) {
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        if (currentUrl !== lastUrl) {
            logger_1.logger.info(`[auth-session] Post-OTP navigation → ${currentUrl}`);
            lastUrl = currentUrl;
        ...
        let hostMatch = false;
        try {
            hostMatch = new URL(currentUrl).hostname === targetHost;
        }
        catch { /* ignore */ }
        ...
        if (onAuthPage) {
            ...
            if (errorText)
                throw new Error(`Sky reported an OTP error: ${errorText.slice(0, 200)}`);
        }
...
async function cleanupBrowser(session) {
    try {
...
setInterval(() => {
    ...
    for (const [id, session] of sessions) {
        if (session.expiresAt < now && session.phase !== "authenticated") {
            logger_1.logger.info(`[auth-session ${id}] Expired (TTL), cleaning up`);
            session.phase = "expired";
            void cleanupBrowser(session);
            sessions.delete(id);
        }

...
exports.issueRouter.use(auth_1.authenticate);
// GET /api/issues?scan_id=...&severity=...&page=...
exports.issueRouter.get("/", async (req, res) => {
    const { scan_id, severity, category, rule_id, priority, is_resolved, include_advisory, page: pageQ, limit: limitQ } = req.query;
    ...
    if (include_advisory !== "true" && category !== "advisory") {
        conditions.push(`COALESCE(category, '') <> 'advisory'`);
    }
    ...
    const [rows, count] = await Promise.all([
        db_1.db.query(q, params),
        db_1.db.query(`SELECT COUNT(*) FROM issues ${where}`, params.slice(0, -2))
    ]);
    ...
    const priorityRank = (sev) => sev === "critical" ? 1 : sev === "serious" ? 2 : sev === "moderate" ? 3 : 4;
    const grouped = [];
...
// GET /api/issues/:id
exports.issueRouter.get("/:id", async (req, res) => {
    const result = await db_1.db.query("SELECT * FROM issues WHERE id = $1", [req.params.id]);
    if (!result.rows[0]) {
...
exports.issueRouter.post("/:id/ai-explain", async (req, res) => {
    const result = await db_1.db.query("SELECT * FROM issues WHERE id = $1", [req.params.id]);
    ...
    const ai = await aiService_1.aiService.explainIssue(issue);
    await db_1.db.query("UPDATE issues SET ai_explanation = $1, ai_impact = $2, ai_fix_code = $3 WHERE id = $4", [ai.explanation, ai.impact, ai.fix_code, issue.id]);
    res.json(ai); // ai already includes { source, fallback_reason? }
});
// PATCH /api/issues/:id
exports.issueRouter.patch("/:id", async (req, res) => {
    const { is_resolved, false_positive, priority } = req.body;
    const sets = [];
    const params = [];
    if (typeof is_resolved === "boolean") {
        params.push(is_resolved);
        sets.push(`is_resolved = $${params.length}`);
    }
    if (typeof false_positive === "boolean") {
        params.push(false_positive);
        sets.push(`false_positive = $${params.length}`);
    }
    if (priority) {
    ...
    const result = await db_1.db.query(`UPDATE issues SET ${sets.join(", ")} WHERE id = $${params.length} RETURNING *`, params);
    const issue = result.rows[0];
    if (!issue) {
        res.status(404).json({ error: "Issue not found" });
        return;
    ...
    if (typeof is_resolved === "boolean" || typeof false_positive === "boolean") {
        const unresolved = await db_1.db.query(`SELECT COUNT(*) AS count FROM issues
       WHERE scan_id = $1 AND rule_id = $2 AND url = $3 AND is_resolved = $4 AND false_positive = $5`, [issue.scan_id, issue.rule_id, issue.url, false, false]);
        const nextStatus = Number(unresolved.rows[0]?.count || 0) === 0 ? "pass" : "fail";
        await db_1.db.query(`UPDATE test_cases SET status = $1
       WHERE scan_id = $2
         AND category NOT IN ('manual-review', 'hybrid-review')
         AND (issue_id = $3 OR (description LIKE $4 AND description LIKE $5))`, [nextStatus, issue.scan_id, issue.id, `%${issue.rule_id}%`, `%${issue.url}%`]);
    }
    res.json({ issue });
});

...
export type WcagRuleRegistryRow = {
  rule_id: string;
  rule_name: string;
  category?: string;
  default_wcag: any;
  approved_wcag: any;
  mapping_status: "approved" | "review_required" | "rejected" | "obsolete" | "advisory";
  review_status: "pending" | "approved" | "rejected" | "resolved";
...
const APPROVED_RULE_MAPPINGS: Array<{
  ruleId: string;
  ruleName: string;
  category: string;
  wcag: string[];
...
export function normalizeWcagCriterion(value: string): string | null {
  ...
  if (dotted) return dotted === "4.1.1" ? null : dotted;
...
export function normalizeWcagCriteria(values: string[]): string[] {
  return Array.from(new Set(values.map(normalizeWcagCriterion).filter((value): value is string => Boolean(value))))
    .filter((criterion) => ACTIVE_WCAG_22.has(criterion))
    .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
}

function sourceModuleFor(issue: ScanIssue): string {
  if (issue.sourceHint) return issue.sourceHint;
  if (issue.ruleId.startsWith("axe:")) return "axeScan";
  return issue.ruleId.split(":")[0] || "scanner";
}

async function insertDecision(ruleId: string, previousWcag: string[], decidedWcag: string[], decision: string, reason?: string, actorId?: string): Promise<void> {
  await db.query(
    `INSERT INTO wcag_mapping_decisions (rule_id, previous_wcag, decided_wcag, decision, reason, decided_by)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [ruleId, JSON.stringify(previousWcag), JSON.stringify(decidedWcag), decision, reason || null, actorId || null]
  );
}

export async function seedApprovedRuleMappings(): Promise<void> {
  for (const item of APPROVED_RULE_MAPPINGS) {
    const approved = normalizeWcagCriteria(item.wcag);
    await db.query(
      `INSERT INTO wcag_rule_registry
      ...
      [item.ruleId, item.ruleName, item.category, JSON.stringify(approved), item.sourceModule, item.rationale]
    );
  }
}

export async function registerRuleMappingProposal(issue: ScanIssue): Promise<WcagRuleRegistryRow> {
  ...
  const rows = await db.query<WcagRuleRegistryRow>("SELECT * FROM wcag_rule_registry WHERE rule_id = $1", [issue.ruleId]);
  return rows.rows[0];
}

export async function resolveGovernedWcagMapping(issue: ScanIssue): Promise<{ wcag: string[]; mappingStatus: string; registry: WcagRuleRegistryRow }> {
...
export async function approveRuleMappingFromReview(ruleId: string, criteria: string[], reason: string, actorId?: string): Promise<void> {
  const approved = normalizeWcagCriteria(criteria);
  if (!approved.length) throw new Error("Cannot approve a WCAG mapping without active WCAG 2.2 criteria.");
  const existing = await db.query<WcagRuleRegistryRow>("SELECT * FROM wcag_rule_registry WHERE rule_id = $1", [ruleId]);
  ...
  await db.query(
    `INSERT INTO wcag_rule_registry
     ...
     ON CONFLICT(rule_id) DO UPDATE SET
       ...
       updated_at = datetime('now')`,
    [ruleId, JSON.stringify(approved), reason, actorId || null]
  );
  await insertDecision(ruleId, previous, approved, "accepted", reason, actorId);
}

export async function dismissRuleMappingReview(ruleId: string, criteria: string[], reason: string, actorId?: string): Promise<void> {
  const rejected = normalizeWcagCriteria(criteria);
  const existing = await db.query<WcagRuleRegistryRow>("SELECT * FROM wcag_rule_registry WHERE rule_id = $1", [ruleId]);
  const previous = normalizeWcagCriteria(parseMaybeJsonArray(existing.rows[0]?.approved_wcag));
  await db.query(
    `UPDATE wcag_rule_registry
     SET review_status = CASE WHEN approved_wcag = '[]' THEN 'rejected' ELSE 'resolved' END,
         ...
         updated_at = datetime('now')
     WHERE rule_id = $1`,
    [ruleId, reason, actorId || null]
  );
  await insertDecision(ruleId, previous, rejected, "dismissed", reason, actorId);
}

export async function resolveRuleMappingReview(ruleId: string, criteria: string[], reason: string, actorId?: string): Promise<void> {
  const rows = await db.query<WcagRuleRegistryRow>("SELECT * FROM wcag_rule_registry WHERE rule_id = $1", [ruleId]);
  const registry = rows.rows[0];
  if (!registry) throw new Error(`Rule ${ruleId} is not registered in WCAG governance.`);

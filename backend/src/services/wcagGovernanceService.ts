...
import {
  activeWcag22CriteriaCount,
  approveRuleMappingFromReview,
  dismissRuleMappingReview,
  resolveRuleMappingReview,
  seedApprovedRuleMappings
} from "./wcagRuleRegistryService";

const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
const W3C_QUICKREF_URL = "https://www.w3.org/WAI/WCAG22/quickref/";
let governanceRun: Promise<void> | null = null;
...
const FALLBACK_WCAG: WcagMetadata[] = [
...
function normalizeCriterion(value: string): string | null {
  const raw = String(value || "").trim().toLowerCase();
  if (!raw) return null;

  const withoutPrefix = raw.replace(/^wcag/, "");
  if (/^(2|20|21|22)(a|aa|aaa)$/.test(withoutPrefix)) return null;
...
function normalizeCriteria(values: string[]): string[] {
  return Array.from(
    new Set(values.map((value) => normalizeCriterion(value)).filter((value): value is string => Boolean(value)))
...
function inferLevel(text: string): "A" | "AA" | "AAA" {
...
function preferredCriterionTitle(criterion: string, parsedTitle: string, slug: string): string {
  const fallback = FALLBACK_WCAG.find((item) => item.criterion === criterion)?.title;
  if (fallback) return fallback;
  if (!parsedTitle || /^understanding$/i.test(parsedTitle)) return titleFromSlug(slug);
  return parsedTitle;
}

function parseWcagQuickref(html: string): WcagMetadata[] {
  const found = new Map<string, WcagMetadata>();
...
async function seedMetadata(rows: WcagMetadata[], source: string): Promise<void> {
  for (const item of rows) {
    const title = preferredCriterionTitle(item.criterion, item.title, item.url.split("/").pop()?.replace(/\.html.*/i, "") || "");
    await db.query(
      `INSERT INTO wcag_metadata (criterion, title, level, principle, url, source, fetched_at)
      ...
      [item.criterion, title, item.level, item.principle, item.url, source]
    );
  }
}

async function fetchLiveWcagMetadata(): Promise<WcagMetadata[]> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 10000);
  try {
    const response = await fetch(W3C_QUICKREF_URL, { signal: controller.signal });
    if (!response.ok) throw new Error(`W3C quickref returned ${response.status}`);
    ...
    logger.warn(`WCAG quickref parse returned ${parsed.length} rows; using bundled metadata with live source verification.`);
...
async function refreshIfNeeded(force = false): Promise<{ refreshed: boolean; source: string }> {
  const latest = await db.query<{ fetched_at: string }>("SELECT MAX(fetched_at) AS fetched_at FROM wcag_metadata");
  ...
  if (!force && !stale) return { refreshed: false, source: "current-cache" };
  ...
  try {
    ...
    logger.warn("WCAG live metadata refresh failed; using bundled fallback metadata.", error);
    await seedMetadata(FALLBACK_WCAG, "bundled-fallback");
    return { refreshed: true, source: "bundled-fallback" };
  }
}

function suspiciousReason(ruleId: string, title: string, wcag: string, criterionTitle?: string): string | null {
...
async function criterionTitle(wcag: string): Promise<string | undefined> {
  ...
  const rows = await db.query<{ title: string }>("SELECT title FROM wcag_metadata WHERE criterion = $1", [criterion]);
  return rows.rows[0]?.title;
}

async function validateLocalRules(): Promise<void> {
  for (const rule of LOCAL_RULES) {
    ...
    for (const current of currentCriteria) {
      ...
      if (reason) await upsertReview(rule.ruleId, currentCriteria, suggestedCriteria, reason);
    }
  }
}

async function validateHistoricalIssues(): Promise<void> {
  const rows = await db.query<{ rule_id: string; wcag_criteria: any; message: string }>(
    `SELECT rule_id, wcag_criteria, message
     FROM issues
...
async function resolveStaleNormalizedReviewRows(): Promise<void> {
  const rows = await db.query<{ id: string; current_wcag: any }>(
    `SELECT id, current_wcag
  ...
  for (const row of rows.rows) {
    ...
    if (criteria.length && missing) continue;
      ...
      if (currentJson !== JSON.stringify(parseMaybeJsonArray(row.current_wcag)) || suggestedJson !== JSON.stringify(parseMaybeJsonArray(row.suggested_wcag))) {
        const collision = await db.query<{ id: string }>(
          `SELECT id
           ...
           LIMIT 1`,
          [row.rule_id, currentJson, row.reason, row.id]
        );
...
async function runWcagGovernance(forceRefresh: boolean): Promise<void> {
  const result = await refreshIfNeeded(forceRefresh);
  if (result.refreshed) logger.info(`WCAG metadata refreshed from ${result.source}`);
...
export async function ensureWcagGovernanceReady(forceRefresh = false): Promise<void> {
  if (governanceRun) {
    await governanceRun;
  ...
  try {
    ...
    if (governanceRun === currentRun) governanceRun = null;
  }
}

export async function getWcagGovernanceStatus(): Promise<any> {
  await ensureWcagGovernanceReady();
  const meta = await db.query<{ fetched_at: string; source: string }>(
    "SELECT fetched_at, source FROM wcag_metadata ORDER BY fetched_at DESC LIMIT 1"
  );
  const criteria = await db.query<{ count: number }>("SELECT COUNT(*) AS count FROM wcag_metadata");
  const pending = await db.query<{ count: number }>(
    "SELECT COUNT(*) AS count FROM wcag_mapping_reviews WHERE status = 'pending'"
  );
  const reviews = await db.query(
    "SELECT * FROM wcag_mapping_reviews WHERE status = 'pending' ORDER BY last_seen_at DESC LIMIT 5"
  );
...
export async function listWcagMappingReviews(status = "pending"): Promise<any[]> {
  await ensureWcagGovernanceReady();
  const rows = await db.query("SELECT * FROM wcag_mapping_reviews WHERE status = $1 ORDER BY last_seen_at DESC", [status]);
...
export async function updateWcagMappingReview(id: string, status: string, actorId?: string): Promise<any> {
  if (!["pending", "accepted", "dismissed", "resolved"].includes(status)) {
    throw new Error("Invalid WCAG review status");
  }
  const before = await db.query("SELECT * FROM wcag_mapping_reviews WHERE id = $1", [id]);
  ...
  if (status === "accepted") {
    await approveRuleMappingFromReview(review.rule_id, suggested.length ? suggested : current, reason, actorId);
  } else if (status === "dismissed") {
    await dismissRuleMappingReview(review.rule_id, suggested.length ? suggested : current, reason, actorId);
  } else if (status === "resolved") {
    await resolveRuleMappingReview(review.rule_id, current, reason, actorId);
  }
  ...
  const rows = await db.query("SELECT * FROM wcag_mapping_reviews WHERE id = $1", [id]);

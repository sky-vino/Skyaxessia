...
issueRouter.get("/", async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  res.json({ issues: rows.rows, total: Number(count.rows[0].count), page, limit });
});
...
issueRouter.post("/:id/ai-explain", async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (issue.ai_explanation) {
    res.json({ explanation: issue.ai_explanation, impact: issue.ai_impact, fix_code: issue.ai_fix_code });
    return;
  }

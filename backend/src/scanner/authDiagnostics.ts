...
import type { Page, BrowserContext, Request, Response, ConsoleMessage } from "playwright";
...
interface CookieSnapshot {
  ...
  path: string;
  valueLength: number;
  valueHash: string;    // Never log the raw value.
  expires: number;
  httpOnly: boolean;
  secure: boolean;
  sameSite: string;
...
export class AuthDiagnostics {
  ...
  private consoleEvents: CapturedConsole[] = [];
  private framenavs: { url: string; at: number }[] = [];
  ...
  private onResponse = (res: Response) => {
    try {
      this.responses.push({
        ...
        fromCache: (res as any).fromCache ? Boolean((res as any).fromCache()) : false,
        timing: null,
        headers: res.headers(),
        serverAddr: null
      });
  ...
  private onFrameNavigated = (frame: any) => {
    try {
  ...
  constructor(private page: Page, private scanId: string, private label: string) {
    this.context = page.context();
  }

  async startCapture(): Promise<void> {
    this.startedAt = Date.now();
    logger.info(`[NET-DIAG] Starting capture for label="${this.label}" scan=${this.scanId}`);
    ...
    try {
      ...
      logger.info(`[TRACE-DIAG] Playwright tracing started for label="${this.label}"`);
    } catch (err: any) {
      logger.warn(`[TRACE-DIAG] Could not start tracing: ${err?.message || err}`);
    }
    ...
    try {
      ...
      logger.info(`[COOKIE-DIAG] Before ${this.label}: ${this.cookiesBefore.length} cookies present`);
      for (const c of this.cookiesBefore) {
        logger.info(`[COOKIE-DIAG]   ${c.domain}${c.path} ${c.name} (${c.valueLength}B hash=${c.valueHash} httpOnly=${c.httpOnly} secure=${c.secure} sameSite=${c.sameSite})`);
      }
    } catch (err: any) {
      logger.warn(`[COOKIE-DIAG] Could not snapshot cookies before: ${err?.message || err}`);
    }
  }

  async stopAndLog(): Promise<{ tracePath?: string }> {
    const durationMs = Date.now() - this.startedAt;
    logger.info(`[NET-DIAG] Stopping capture for label="${this.label}" after ${durationMs}ms`);

    // Detach listeners.
    try { this.page.off("request", this.onRequest); } catch { /* ignore */ }
    try { this.page.off("response", this.onResponse); } catch { /* ignore */ }
    try { this.page.off("console", this.onConsole); } catch { /* ignore */ }
    try { this.page.off("framenavigated", this.onFrameNavigated); } catch { /* ignore */ }

    // Log frame navigations (redirect chain, in order).
    for (const nav of this.framenavs) {
      logger.info(`[NET-DIAG] Frame navigated @+${nav.at}ms → ${nav.url}`);
    }

    // Log interesting requests/responses (mostly filter static assets).
    const interesting = this.buildRedirectChain();
    logger.info(`[NET-DIAG] Captured ${this.requests.length} requests, ${this.responses.length} responses, ${interesting.length} interesting`);

    for (const item of interesting) {
      logger.info(`[NET-DIAG] ────────────────────────────────────────────────────`);
      logger.info(`[NET-DIAG] ${item.request.method} ${item.request.url}`);
      logger.info(`[NET-DIAG]   isNavigation:  ${item.request.isNavigationRequest}`);
      logger.info(`[NET-DIAG]   resourceType:  ${item.request.resourceType}`);
      logger.info(`[NET-DIAG]   postBodySize:  ${item.request.postDataSize ?? "<none>"}`);
      logger.info(`[NET-DIAG]   reqHeaders:`);
      for (const [k, v] of Object.entries(item.request.headers)) {
        if (this.isSafeHeaderToLog(k)) {
          logger.info(`[NET-DIAG]     ${k}: ${this.truncate(v, 300)}`);
        }
      }
      if (!item.response) {
        logger.info(`[NET-DIAG]   → NO RESPONSE (request pending or connection failed)`);
        continue;
      }
      logger.info(`[NET-DIAG]   → status:     ${item.response.status} ${item.response.statusText}`);
      logger.info(`[NET-DIAG]   → fromCache:  ${item.response.fromCache}`);
      const location = item.response.headers.location || item.response.headers.Location;
      if (location) {
        logger.info(`[NET-DIAG]   → redirect:   ${location}`);
      }
      logger.info(`[NET-DIAG]   → respHeaders:`);
      ...
      const markers = cfMarkers
        ...
        .filter(([, v]) => v);
      if (markers.length) {
        logger.info(`[NET-DIAG]   → WAF/CDN markers:`);
        for (const [k, v] of markers) {
          logger.info(`[NET-DIAG]     ${k}: ${v}`);
        }
      }
    }

    // Log console events.
    logger.info(`[CONSOLE-DIAG] ${this.consoleEvents.length} browser console events captured`);
    for (const evt of this.consoleEvents) {
      const prefix = evt.type === "error" ? "[CONSOLE-DIAG-ERR]" : "[CONSOLE-DIAG]";
      logger.info(`${prefix}   [${evt.type}] ${evt.text}${evt.location ? ` @ ${evt.location}` : ""}`);
    }
    ...
    try {
      ...
      logger.info(`[COOKIE-DIAG] After ${this.label}: ${cookiesAfter.length} cookies present`);
      const beforeKey = (c: CookieSnapshot) => `${c.domain}|${c.path}|${c.name}`;
      const beforeMap = new Map(this.cookiesBefore.map(c => [beforeKey(c), c] as const));
      const afterMap = new Map(cookiesAfter.map(c => [beforeKey(c), c] as const));
      ...
      logger.info(`[COOKIE-DIAG]   Δ added=${added.length} removed=${removed.length} changed=${changed.length}`);
      for (const c of added)   logger.info(`[COOKIE-DIAG]   +  ${c.domain}${c.path} ${c.name} (${c.valueLength}B hash=${c.valueHash})`);
      for (const c of removed) logger.info(`[COOKIE-DIAG]   -  ${c.domain}${c.path} ${c.name}`);
      for (const c of changed) logger.info(`[COOKIE-DIAG]   ~  ${c.domain}${c.path} ${c.name} (new hash=${c.valueHash}, ${c.valueLength}B)`);
    } catch (err: any) {
      logger.warn(`[COOKIE-DIAG] Could not snapshot cookies after: ${err?.message || err}`);
    }
    ...
    try {
      ...
      logger.info(`[PAGE-DIAG] After ${this.label}: url=${url}`);
      logger.info(`[PAGE-DIAG] After ${this.label}: title="${title}"`);
      ...
      logger.info(`[PAGE-DIAG]   visible forms: ${pageProbe.forms}`);
      logger.info(`[PAGE-DIAG]   visible inputs: ${pageProbe.inputs}`);
      logger.info(`[PAGE-DIAG]   visible buttons: ${pageProbe.buttons}`);
      if (pageProbe.errors.length) {
        logger.info(`[PAGE-DIAG]   visible error messages: ${pageProbe.errors.length}`);
        for (const err of pageProbe.errors) logger.info(`[PAGE-DIAG]     error: ${err}`);
      }
      logger.info(`[PAGE-DIAG]   body text (first 800 chars): ${pageProbe.bodyText}`);
    } catch (err: any) {
      logger.warn(`[PAGE-DIAG] Could not probe page: ${err?.message || err}`);
    }
    ...
    if (this.tracingActive) {
      try {
        ...
        this.tracePath = path.join(
          TRACE_DIR,
          `scan-${this.scanId}-${this.label}-${Date.now()}.zip`
        );
        await this.context.tracing.stop({ path: this.tracePath });
        const stat = await fs.promises.stat(this.tracePath).catch(() => null);
        logger.info(`[TRACE-DIAG] Playwright trace saved: ${this.tracePath} (${stat?.size ?? "?"} bytes)`);
        logger.info(`[TRACE-DIAG]   Local viewer:   npx playwright show-trace <downloaded-file>`);
        logger.info(`[TRACE-DIAG]   Online viewer:  https://trace.playwright.dev`);
        logger.info(`[TRACE-DIAG]   Download URL:   GET /api/scans/${this.scanId}/auth-trace?label=${encodeURIComponent(this.label)}`);
      } catch (err: any) {
        logger.warn(`[TRACE-DIAG] Failed to save trace: ${err?.message || err}`);
      }
    }
  ...
  private hashCookie = (c: any): CookieSnapshot => {
   ...
   */
  private buildRedirectChain(): { request: CapturedRequest; response?: CapturedResponse }[] {
    const responsesByUrl = new Map<string, CapturedResponse>();
    for (const r of this.responses) responsesByUrl.set(r.url, r);

    const pairs: { request: CapturedRequest; response?: CapturedResponse }[] = [];
  ...
  private isSafeHeaderToLog(name: string): boolean {
    ...
    if (lower === "proxy-authorization") return false;
...
function simpleHash8(input: string): string {
  let h = 2166136261;

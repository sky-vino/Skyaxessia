...
function IssueCard({ issue, focused, onBackToIssue }: { issue: any; focused?: boolean; onBackToIssue?: (issueId: string) => void }) {
  ...
  const ai = aiData || { explanation: issue.ai_explanation, impact: issue.ai_impact, fix_code: issue.ai_fix_code, source: issue.ai_source, fallback_reason: issue.ai_fallback_reason };
  ...
  // Tier 4 fix — surface whether the AI call really succeeded or the canned
  // fallback fired. Previously the fallback strings were rendered as if the
  // AI had responded, so users couldn't tell.
  const isFallback = ai.source === "fallback"
    || (ai.fix_code && /Please refer to WCAG documentation/i.test(ai.fix_code));
      ...
      <AnimatePresence>
        {expanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden border-t" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          <div className="p-5 space-y-5">
            {hasAi && isFallback && (
              <div className="p-3 rounded-xl" style={{ background: "rgba(255,159,67,0.08)", border: "1px solid rgba(255,159,67,0.28)" }}>
                <div className="text-xs font-semibold text-orange-300 mb-1">Not AI-generated (fallback shown)</div>
                <p className="text-xs text-slate-400 leading-relaxed">
                  The AI service was not reachable when this fix was requested — the text below is a generic fallback, not model output. Regenerate to try the AI again.
                </p>
              </div>
            )}
            {hasAi ? <>
